...
UCHAR rgFirstInfo[256];
   //DoCheckDisk(FALSE);
#if 0 //f32mount required qemuimage - will remove this section later
   //ProcessFsTab(FALSE);
#endif
   //return 0;
...
#endif //f32mount required qemuimage - will remove this section later
	  ...
	  if (rgArgv[iArg][0] == '/' || rgArgv[iArg][0] == '-')
		 ...
		 switch (rgArgv[iArg][1])
			...
			case '?' :
			   ...
			   printf("/Q (Quit)\n");
			   printf("/N do NOT load lazy write daemon.\n");
			   ...
			   printf("/F force lazy write daemon to be loaded\n");
			   DosExit(EXIT_PROCESS, 0);
			   break;
   ...
   if (!fActive)
	  ...
	  if (!ulDriveMap)
		 ...
		 if( !f32Parms.fForceLoad )
			...
			printf("FAT32: Use /F option to load lazy write daemon\n");
			DosExit(EXIT_PROCESS, 1);
			}
		 }
	  }
