...
APIRET ModifyDirectory(PCDINFO pCD, ULONG ulDirCluster, PSHOPENINFO pDirSHInfo,
                       USHORT usMode, PDIRENTRY pOld, PDIRENTRY pNew, PSZ pszLongNameOld, PSZ pszLongNameNew);
...
static void usage(char *s)
{
   show_message("\nUsage: [c:\\] %s x: [/v[:[1|2]] [/p] [/f] [/c] [/a] [/h] [/?]]\n"
                "\n"
                "(c) Henk Kelder, Netlabs & Arca Noae, covered by (L)GPL\n"
...
struct extbpb dp;
   ...
   if (pCD->bFatType)
      {
      // try checking only FAT32/FAT16/FAT12 drives
      rc = ChkDskMain(pCD);
      }
   else
      {
            ...
            */
            ...
            if (f32Parms.fEAS && HAS_EAS( pDir->fEAS ))
               ...
               if (ulDirCluster == pCD->ulFatEof)
                  ...
                  {
                  ulFileCluster = FindPathCluster(pCD, ulDirCluster, pszFile, NULL, &DirEntry, NULL);
               ...
               if (f32Parms.fEAS && pEA && ullSize)
                  ...
                  if (rc == 2 || rc == 3)
                     ...
                     if (pCD->fFix)
                        ...
                        if (!rc)
                           {
                           ulDstFileCluster = FindPathCluster(pCD, ulDirCluster, Mark.szFileName, NULL, &DstDirEntry, NULL);
                           ...
                           if (rc == ERROR_FILE_NOT_FOUND)
                              ...
                              rc = ModifyDirectory(pCD, ulDirCluster, NULL, MODIFY_DIR_RENAME,
                                                   &DirEntry, &DirNew, pszFile, Mark.szFileName);
                              }
                           }
                        if (!rc)
