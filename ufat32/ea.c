...
USHORT rc;
   ...
   ulCluster = FindPathCluster(pCD, ulDirCluster, pszFileName, pDirSHInfo, pOldEntry, NULL);
   ...
   rc = ModifyDirectory(pCD, ulDirCluster, pDirSHInfo,
      MODIFY_DIR_UPDATE, pOldEntry, pNewEntry, pszFileName, pszFileName);
...
BOOL fFirst = TRUE;
   ...
   *ppFEAL = NULL;
   ...
   ulCluster = FindPathCluster(pCD, ulDirCluster, pszEAName, pDirSHInfo, NULL, NULL);
   free(pszEAName);
...
ULONG    ulCluster;
   ...
   ulCluster = FindPathCluster(pCD, ulDirCluster, pszEAName, pDirSHInfo, pDirEntry, NULL);
   if (ulCluster == pCD->ulFatEof)
      {
      rc = 0;
      goto usDeleteEASExit;
      }
   rc = ModifyDirectory(pCD, ulDirCluster, pDirSHInfo, MODIFY_DIR_DELETE, pDirEntry, NULL, pszEAName, pszEAName);
   if (rc)
      goto usDeleteEASExit;
...
PSHOPENINFO pSHInfo = NULL;
   ...
   if (!ulCluster || ulCluster == pCD->ulFatEof)
      ...
      if (fNew)
         rc = MakeDirEntry(pCD, ulDirCluster, pDirSHInfo, pDirNew, pszEAName);
      else
         rc = ModifyDirectory(pCD, ulDirCluster, pDirSHInfo,
            MODIFY_DIR_UPDATE, pDirEntry, pDirNew, pszEAName, pszEAName);
      if (rc)
         {
         free(pszEAName);
         goto usWriteEASExit;
      ...
      rc = ModifyDirectory(pCD, ulDirCluster, pDirSHInfo, MODIFY_DIR_UPDATE,
           pDirEntry, pDirNew, pszEAName, pszEAName);
      if (rc)
         {
         free(pszEAName);
         goto usWriteEASExit;
