...
PSHOPENINFO pSrcSHInfo = NULL;
   ...
   ulSrcCluster = FindPathCluster(pVolInfo, ulSrcDirCluster, pszSrcEAName, pDirSrcSHInfo, pSrcEntry, NULL);
   ulTarCluster = FindPathCluster(pVolInfo, ulTarDirCluster, pszTarEAName, pDirTarSHInfo, pTarEntry, NULL);
   if (ulTarCluster != pVolInfo->ulFatEof)
      {
      rc = ModifyDirectory(pVolInfo, ulTarDirCluster, pDirTarSHInfo, MODIFY_DIR_DELETE, pTarEntry, NULL, pszTarEAName, pszTarEAName, 0);
      //rc = ModifyDirectory(pVolInfo, ulTarDirCluster, pDirTarSHInfo, MODIFY_DIR_DELETE, pTarEntry, NULL, NULL, 0);
      if (rc)
         goto usCopyEASExit;
   ...
   /*
      Make new direntry
   */
   rc = ModifyDirectory(pVolInfo, ulTarDirCluster, pDirTarSHInfo, MODIFY_DIR_INSERT, NULL, pSrcEntry, pszTarEAName, pszTarEAName, 0);
...
PDIRENTRY pSrcEntry = NULL, pTarEntry = NULL;
   ...
   if (ulSrcDirCluster == ulTarDirCluster)
      ...
      rc = ModifyDirectory(pVolInfo, ulSrcDirCluster, pDirSrcSHInfo,
         MODIFY_DIR_RENAME, pSrcEntry, pTarEntry, pszSrcEAName, pszTarEAName, 0);
      goto usMoveEASExit;
      }

   rc = ModifyDirectory(pVolInfo, ulSrcDirCluster, pDirSrcSHInfo, MODIFY_DIR_DELETE, pSrcEntry, NULL, pszSrcEAName, pszSrcEAName, 0);
   //rc = ModifyDirectory(pVolInfo, ulSrcDirCluster, pDirSrcSHInfo, MODIFY_DIR_DELETE, pSrcEntry, NULL, NULL, 0);
   if (rc)
      goto usMoveEASExit;
...
USHORT rc;
   ...
   ulCluster = FindPathCluster(pVolInfo, ulDirCluster, pszFileName, pDirSHInfo, pOldEntry, NULL);
   ...
   rc = ModifyDirectory(pVolInfo, ulDirCluster, pDirSHInfo,
      MODIFY_DIR_UPDATE, pOldEntry, pNewEntry, pszFileName, pszFileName, 0);
   //rc = ModifyDirectory(pVolInfo, ulDirCluster, pDirSHInfo,
   //   MODIFY_DIR_UPDATE, pOldEntry, pNewEntry, NULL, 0);
...
BOOL fFirst = TRUE;
   ...
   *ppFEAL = NULL;
   ...
   ulCluster = FindPathCluster(pVolInfo, ulDirCluster, pszEAName, pDirSHInfo, NULL, NULL);
   free(pszEAName);
...
ULONG    ulCluster;
   ...
   ulCluster = FindPathCluster(pVolInfo, ulDirCluster, pszEAName, pDirSHInfo, pDirEntry, NULL);
   if (ulCluster == pVolInfo->ulFatEof)
      {
      rc = 0;
      goto usDeleteEASExit;
      }
   rc = ModifyDirectory(pVolInfo, ulDirCluster, pDirSHInfo, MODIFY_DIR_DELETE, pDirEntry, NULL, pszEAName, pszEAName, 0);
   //rc = ModifyDirectory(pVolInfo, ulDirCluster, pDirSHInfo, MODIFY_DIR_DELETE, pDirEntry, NULL, NULL, 0);
   if (rc)
      goto usDeleteEASExit;
...
PSHOPENINFO pSHInfo = NULL;
   ...
   if (!ulCluster || ulCluster == pVolInfo->ulFatEof)
      ...
      if (fNew)
         rc = MakeDirEntry(pVolInfo, ulDirCluster, pDirSHInfo, pDirNew, pszEAName);
      else
         rc = ModifyDirectory(pVolInfo, ulDirCluster, pDirSHInfo,
            MODIFY_DIR_UPDATE, pDirEntry, pDirNew, pszEAName, pszEAName, 0);
         //rc = ModifyDirectory(pVolInfo, ulDirCluster, pDirSHInfo,
         //   MODIFY_DIR_UPDATE, pDirEntry, pDirNew, NULL, 0);
      if (rc)
         {
         free(pszEAName);
         goto usWriteEASExit;
      ...
      rc = ModifyDirectory(pVolInfo, ulDirCluster, pDirSHInfo, MODIFY_DIR_UPDATE,
           pDirEntry, pDirNew, pszEAName, pszEAName, 0);
      //rc = ModifyDirectory(pVolInfo, ulDirCluster, pDirSHInfo, MODIFY_DIR_UPDATE,
      //     pDirEntry, pDirNew, NULL, 0);
      if (rc)
         {
         free(pszEAName);
         goto usWriteEASExit;
