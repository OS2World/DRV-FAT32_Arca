   ...
   if (!(ulOpenMode & OPEN_FLAGS_DASD))
      ...
      if (ulCluster == pVolInfo->ulFatEof)
         ...
         ulCluster = 0;
            ...
            if (f32Parms.fLargeFiles)
               {
               if (size > (ULONGLONG)ULONG_MAX && !f32Parms.fFatPlus)
            ...
            if (f32Parms.fLargeFiles)
               {
               if (GreaterUL(size, ULONG_MAX) && !f32Parms.fFatPlus)
         ...
         rc = MakeDirEntry(pVolInfo, ulDirCluster, pDirSHInfo, pDirEntry, pszFile);
         if (rc)
            {
            free(szLongName);
            goto FS_OPENCREATEEXIT;
         ...
         *pAction   = FILE_CREATED;
         ...
         memcpy(pDirOld, pDirEntry, sizeof (DIRENTRY));
            ...
            pDirEntry->fEAS &= FILE_NO_EAS_MASK;
         ...
         if (GreaterUL(size, 0))
            ...
            if (ulCluster != pVolInfo->ulFatEof)
               ...
               rc = ModifyDirectory(pVolInfo, ulDirCluster, pDirSHInfo, MODIFY_DIR_UPDATE,
                  pDirOld, pDirEntry, pszFile, pszFile, usIOMode);
               //rc = ModifyDirectory(pVolInfo, ulDirCluster, pDirSHInfo, MODIFY_DIR_UPDATE,
               //   pDirOld, pDirEntry, NULL, usIOMode);
               if (!rc)
                  rc = ERROR_DISK_FULL;
               free(pDirOld);
               free(szLongName);
      ...
      pOpenInfo->ulCurBlock = 0;
         ...
         FileGetSize(pVolInfo, pDirEntry, pOpenInfo->pSHInfo->ulDirCluster, pOpenInfo->pDirSHInfo, pszFile, &size);
    ...
    else
    {
        ...
        */
        ...
        if ( (! f32Parms.fLargeFiles && (pos + usBytesToWrite >= (LONGLONG)LONG_MAX   ||
             ...
             (f32Parms.fLargeFiles && !f32Parms.fFatPlus && (pos + usBytesToWrite >= (LONGLONG)ULONG_MAX  ||
        ...
        if ( (! f32Parms.fLargeFiles && (iGreaterEUL(llPos, LONG_MAX)   ||
             ...
             (f32Parms.fLargeFiles && !f32Parms.fFatPlus && (iGreaterEUL(llPos, ULONG_MAX)  ||
        ...
        if (f32Parms.fLargeFiles && !f32Parms.fFatPlus &&
   ...
   switch (usType)
      ...
      case FS_COMMIT_ALL:
         ...
         memcpy(pDirEntry, pDirOld, sizeof (DIRENTRY));
            ...
            FileSetSize(pVolInfo, pDirEntry, pOpenInfo->pSHInfo->ulDirCluster, pOpenInfo->pDirSHInfo, pszFile, size);
         ...
         rc = ModifyDirectory(pVolInfo, pOpenInfo->pSHInfo->ulDirCluster, pOpenInfo->pDirSHInfo, MODIFY_DIR_UPDATE,
            pDirOld, pDirEntry, pszFile, pszFile, usIOFlag);
         if (!rc)
            psffsi->sfi_tstamp = 0;
         if (pDirEntry)
            free(pDirEntry);
   ...
   if (usFlag == FI_RETRIEVE)
      ...
      if (usLevel == FIL_STANDARD  || usLevel == FIL_QUERYEASIZE ||
         ...
         ulCluster = FindPathCluster(pVolInfo, pOpenInfo->pSHInfo->ulDirCluster,
            pszFile, pOpenInfo->pDirSHInfo, pDirEntry, NULL);
