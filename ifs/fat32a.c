...
PSZ      szDstLongName = NULL;
   ...
   if (ulDstCluster != pVolInfo->ulFatEof)
      ...
      */
      ...
      rc = ModifyDirectory(pVolInfo, ulDstDirCluster, pDirDstSHInfo, MODIFY_DIR_DELETE,
                           pTarEntry, NULL, pszDstFile, pszDstFile, 0);
      if (rc)
         goto FS_COPYEXIT;
   ...
   */
   ...
   rc = ModifyDirectory(pVolInfo, ulDstDirCluster, pDirDstSHInfo, MODIFY_DIR_INSERT, NULL, pTarEntry, pszDstFile, pszDstFile, 0);
   if (rc)
      goto FS_COPYEXIT;
   ...
   */
   ...
   /*
      modify new entry
   */
   rc2 = ModifyDirectory(pVolInfo, ulDstDirCluster, pDirDstSHInfo, MODIFY_DIR_UPDATE, pTarEntry, pDirEntry, pszDstFile, pszDstFile, 0);
   if (rc2 && !rc)
      rc = rc2;
...
PSZ      szLongName = NULL;
   ...
   rc = ModifyDirectory(pVolInfo, ulDirCluster, pDirSHInfo, MODIFY_DIR_DELETE,
                        pDirEntry, NULL, pszFile, pszFile, 0);
   if (rc)
      goto FS_DELETEEXIT;
...
ULONG  ulDirEntries = 0;
   ...
   if (fFound)
      ...
      rc = ModifyDirectory(pVolInfo, pVolInfo->BootSect.bpb.RootDirStrtClus, NULL,
         MODIFY_DIR_INSERT, NULL, pDirEntry, pszVolLabel, pszVolLabel, DVIO_OPWRTHRU);
      }
   if (rc)
      {
      free(pDirEntry);
   ...
   for (usNr = 0; usNr < 9999; usNr++)
      ...
      USHORT usNum = usNr;
         ...
         if (FindPathCluster(pVolInfo, pVolInfo->BootSect.bpb.RootDirStrtClus,
            szFileName, NULL, NULL, NULL) == pVolInfo->ulFatEof)
            break;
      }
   if (usNr == 9999)
      {
...
PSZ      szDstLongName = NULL;
   ...
   */
   ...
   ulTarCluster = FindPathCluster(pVolInfo, ulDstDirCluster, pszDstFile, pDirDstSHInfo, pDirEntry, NULL);
   ...
   */
   ...
   ulCluster = FindPathCluster(pVolInfo, ulSrcDirCluster, pszSrcFile, pDirSrcSHInfo, pDirEntry, NULL);
   if (ulCluster == pVolInfo->ulFatEof)
      {
      rc = ERROR_FILE_NOT_FOUND; //////
      goto FS_MOVEEXIT;
   ...
   */
   ...
   if (ulSrcDirCluster == ulDstDirCluster)
      ...
      rc = ModifyDirectory(pVolInfo, ulSrcDirCluster, pDirSrcSHInfo,
         MODIFY_DIR_RENAME, pDirOld, pDirEntry, pszSrcFile, pszDstFile, 0);
      free(pDirOld);
      goto FS_MOVEEXIT;
      }
   ...
   */
   ...
   rc = ModifyDirectory(pVolInfo, ulSrcDirCluster, pDirSrcSHInfo, MODIFY_DIR_DELETE, pDirEntry, NULL, pszSrcFile, pszSrcFile, 0); // was pszDstFile (originally, NULL)
   if (rc)
      goto FS_MOVEEXIT;
   ...
   */
   ...
   rc = ModifyDirectory(pVolInfo, ulDstDirCluster, pDirDstSHInfo, MODIFY_DIR_INSERT, NULL, pDirEntry, pszDstFile, pszDstFile, 0);
      if (rc)
      goto FS_MOVEEXIT;
   ...
   */
   if ( pDirEntry->bAttr & FILE_DIRECTORY )
      {
      ULONG ulTmp;
      ulTmp = FindPathCluster(pVolInfo, ulCluster, "..", NULL, pDirEntry, NULL);
      if (ulTmp != pVolInfo->ulFatEof)
         ...
         rc = ModifyDirectory(pVolInfo, ulCluster, NULL,
            MODIFY_DIR_UPDATE, pDirEntry, pDirNew, "..", "..", 0);
         free(pDirNew);
         if (rc)
            goto FS_MOVEEXIT;
         }
...
USHORT rc;
   ...
   for (ulCluster = ulFirstCluster; ulCluster < ulLastCluster; ulCluster++)
      ...
      if (!GetFatAccess(pVolInfo, "MakeChain"))
         ...
         ulNewCluster = GetFatEntry(pVolInfo, ulCluster);
         if (ulNewCluster && pVolInfo->bFatType <= FAT_TYPE_FAT32)
            {
            //CritMessage("FAT32:MakeChain:Cluster %lx is not free!", ulCluster);
            Message("ERROR:MakeChain:Cluster %lx is not free!", ulCluster);
            return ERROR_SECTOR_NOT_FOUND;
   ...
   if (usLongName == LONGNAME_MAKE_UNIQUE)
      ...
      for (usNum = 1; usNum < 32000; usNum++)
         ...
         */
         ...
         ulCluster = FindPathCluster(pVolInfo, ulDirCluster, szFileName, NULL, NULL, NULL); ////
         if (ulCluster == pVolInfo->ulFatEof)
            break;
         }
      if (usNum < 32000)
